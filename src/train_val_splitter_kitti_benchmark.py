...
from raw_data_from_mapping import Image, Lidar, ObjectAnnotation, Calibration
import re
import os
import glob
import config
...
class TrainingValDataSplitter:
    def __init__(self, bags, split_rate=0.7, use_raw = False):
        if use_raw:
            self.bags = bags
            self.raw_img = Image(config.cfg.MAPPING_FILE, config.cfg.RAND_MAP)
            self.raw_tracklet_tag_list = list(ObjectAnnotation(config.cfg.MAPPING_FILE, config.cfg.RAND_MAP).files_path_mapping.keys())
    ...
    def split_by_trainva_file(self):
        with open(config.cfg.TRAIN_LIST, 'r') as fTrain:
            self.training_tags = fTrain.read().splitlines()

            # if train_file read successsfully
            with open(config.cfg.VAL_LIST, 'r') as fVal:
